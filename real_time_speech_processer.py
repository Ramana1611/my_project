...
@app.route('/api/record-and-transcribe', methods=['GET'])
def record_and_transcribe():
    try:
        audio, rate = processor.capture_audio(duration=5)
        if audio is None:
            return jsonify({'error': 'Audio capture failed'}), 500
            
        transcription = processor.speech_to_text(audio)
        return jsonify({
            'transcription': transcription,
            'audio_length': len(audio)/rate
        })
    except Exception as e:
        logger.error(f"Record and transcribe error: {str(e)}")
        return jsonify({'error': 'Processing failed'}), 500
